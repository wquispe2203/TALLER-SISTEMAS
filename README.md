...
| --------------------------- | ----------------- | ------------------- | :----: |
| **US-1 / AC-1.1** (Saldo a favor) | Sí → `Módulo de Resultados` | TC-1 | 🟢 Cubierto |
| **US-1 / AC-1.2** (Traslado cubierto) | Sí → `Módulo de Resultados` | TC-2 | 🟢 Cubierto |
| **US-1 / AC-1.3** (Monto pendiente) | Sí → `Módulo de Resultados` | TC-3 | 🟢 Cubierto |
| **US-2 / AC-2.0** (Validación en cascada) | Sí → `Módulo de Validaciones` | TC-4 | 🟢 Cubierto |
| **US-2 / AC-2.1** (Fecha inválida) | Sí → `Módulo de Validaciones` | TC-5 | 🟢 Cubierto |
| **US-2 / AC-2.2** (Estado no permitido) | Sí → `Módulo de Validaciones` | TC-6 | 🟢 Cubierto |
| **US-2 / AC-2.3** (Modalidad inexistente) | Sí → `Módulo de Validaciones` | TC-7 | 🟢 Cubierto |
| **US-3 / AC-3.1** (Mostrar desglose) | Sí → `Módulo de Resultados` | TC-8 | 🟢 Cubierto |
| **US-3 / AC-3.2** (Visibilidad beneficios) | Sí → `Módulo de Resultados` | TC-9 | 🟢 Cubierto |
| **US-3 / AC-3.3** (Copiar resumen) | Sí → `Módulo de Resultados` | TC-10 | 🟢 Cubierto |
| **US-4 / AC-4.1** (Mostrar saldos iniciales) | Sí → `Módulo de Resultados` | TC-11 | 🟢 Cubierto |
| **US-4 / AC-4.2** (Mensaje conclusivo) | Sí → `Módulo de Resultados` | TC-12 | 🟢 Cubierto |
| **CB-1** (Fecha fuera del rango) | Sí → `Módulo de Validaciones` | TC-5 | 🟡 Reutilizado |
| **CB-2** (Ciclo origen igual destino) | Sí → `Motor de Cálculo` | TC-13 | 🟢 Cubierto |
| **CB-3** (Modalidad inexistente) | Sí → `Módulo de Validaciones` | TC-7 | 🟡 Reutilizado |
| **CB-4** (Estado SUSPENDIDO) | Sí → `Módulo de Validaciones` | TC-6 | 🟡 Reutilizado |
| **CB-5** (Estado RETIRADO) | Sí → `Módulo de Validaciones` | TC-6 | 🟡 Reutilizado |
| **CB-6** (Resultado cero) | Sí → `Motor de Cálculo` | TC-2 | 🟡 Reutilizado |
| **CB-7** (Traslado última semana) | Sí → `Motor de Cálculo` | TC-14 | 🟢 Cubierto |
| **CB-8** (Descuento activo) | Sí → `Motor de Cálculo` | TC-15 | 🟢 Cubierto |
| **CB-9** (Beca activa) | Sí → `Motor de Cálculo` | TC-16 | 🟢 Cubierto |

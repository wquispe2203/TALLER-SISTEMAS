...
Only migrate fields that are **`string` in the Swagger definition**. This covers three TypeScript representations:
...
- **Optional strings** — fields typed as `?: string` (no default value)

```ts
// Before
id = '';                      // string in Swagger
status = SomeStatus.DRAFT;    // string in Swagger, enum on the client
priority?: string;            // optional string in Swagger

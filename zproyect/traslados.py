...
def _detalle_semanas_periodo(inicio: date, fin: date, fecha_traslado: date) -> dict:
    ...
    se "cancela": no cuenta como consumida.
    ...
    """
    semanas_totales = _semanas_en_periodo(inicio, fin)
...
def generar_pasos_contado(
    ...
    Reutiliza `calcular_semanas_consumidas` y `contar_semanas_feriado_entre`
    (las mismas funciones que usa `calcular_valor_contado`) para que el
    texto nunca pueda desincronizarse del monto real.
    ...
    semanas_efectivas = duration_weeks + semanas_feriado
    semanas_restantes = semanas_efectivas - semanas_consumidas
    valor = calcular_valor_contado(cash_price, duration_weeks, semanas_consumidas, fecha_inicio, fecha_fin)
    ...
    # ADR-4 §4.4: clamp a [1, semanas_efectivas] — CUOTAS ya clampeaba (_detalle_semanas_periodo),
    # CONTADO no lo hacía; se agrega por consistencia del mismo invariante de negocio.
    semana_actual = max(1, min(indice_semana_actual, semanas_efectivas))
    ...
    pasos = [
        ...
        (
            ...
            "(cuentan igual, haya pasado el traslado antes o después de que ocurran)."
            if semanas_feriado
            else "El ciclo completo no cruza ninguna semana de feriado."
        ),
        f"Semanas efectivas del ciclo = duración ({duration_weeks}) + semanas feriado ({semanas_feriado}) = {semanas_efectivas}.",
        f"Semanas restantes = semanas efectivas ({semanas_efectivas}) − semanas consumidas ({semanas_consumidas}) = {semanas_restantes}.",
        f"Valor por semana = cash_price (S/ {cash_price:.2f}) ÷ semanas efectivas ({semanas_efectivas}) = "
        f"S/ {(cash_price / Decimal(semanas_efectivas)):.4f}.",
        f"Saldo = valor por semana × semanas restantes ({semanas_restantes}) = S/ {valor:.2f} "
        "(redondeo half-up a 2 decimales).",
    ]
    detalle = {
        "semanas_totales": semanas_efectivas,
        "semanas_consumidas": semanas_consumidas,
        "semanas_restantes": semanas_restantes,
...
def generar_pasos_cuotas(
    ...
    if sel["caso"] == "fuera_de_ciclo":
        ...
        # ADR-4 §4.3: sin periodo vigente que describir -> null explícito, no se omite ni se rellena.
        return pasos, {
    ...
    if sel["caso"] == "antes_primera_cuota":
        ...
        # ADR-4 §4.3: no ha empezado ningún periodo -> fecha_inicio_periodo null;
        # fecha_fin_periodo = vencimiento de la cuota 1 (ya se narra arriba); semana_actual = 1.
        return pasos, {
    ...
    pasos.append(
        f"Ese periodo equivale a {d['semanas_totales']} semana(s) "
    ...
    if d["feriado_indices"]:
        pasos.append(
            ...
            "que no cuentan como consumidas."
        )
    else:
    ...
    pasos.append(
        ...
        f"÷ semanas totales del periodo ({d['semanas_totales']}) = S/ {valor:.2f} "
        "(redondeo half-up a 2 decimales)."
    )
...
def calcular_traslado(
    ...
    # PASO 4 y 5: valor residual + desglose narrado ("cálculo humano").
    # `generar_pasos_contado`/`generar_pasos_cuotas` son la ÚNICA fuente de
    # los números que se muestran en `detalle` (FR-010): así el texto
    # explicativo y el monto nunca pueden desincronizarse entre sí. Antes,
    # `detalle` se calculaba aparte con `calcular_semanas_consumidas` para
    # AMBAS modalidades, lo cual era incorrecto para CUOTAS (mostraba las
    # semanas del ciclo completo en vez de las del periodo de la cuota
    # vigente realmente usado en la fórmula) — ver spec.md AC-3.1.
